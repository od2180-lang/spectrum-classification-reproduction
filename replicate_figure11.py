...
class TechnologyClassifClass:
    ...
    def __init__(self, f_start, idsensor, name_sensor):
        ...
        self.BASE_MODEL_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'PSD-technology-classification-framework/TCpackage/resources/')
        ...
        for file in os.listdir(self.detected_transmissions_path):
            ...
            if file.find("results") == -1 and file.endswith('.npy'):
                ...
                try:
                    ...
                    # extract2MHz raises NameError('Error Small BW') for width < 202
                    # Skip this file — it would never be classified
                    pass
...
# ============================================================
# MAIN: Run authors' exact pipeline + evaluate
# ============================================================
...
if __name__ == '__main__':
    ...
    DATA_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'detected_transmissions')
    LABELS = ['dab', 'dvbt', 'fm', 'gsm', 'lte', 'tetra']
    LABEL_TO_IDX = {l: i for i, l in enumerate(LABELS)}

    print("=" * 60)
    print("  Authors' Exact Pipeline (TechClass.py)")
    print("=" * 60)

    print("\n[1/2] Loading models and classifying...")
    classifier = TechnologyClassifClass(f_start=0, idsensor='0', name_sensor='replication')
    classifier.detected_transmissions_path = DATA_DIR + '/'
    labels_results = classifier.inference_trx_labels()

    print(f"\n  Classified: {len(labels_results)} transmissions")

    print("\n[2/2] Evaluating...")
    true_techs = []
    pred_techs = []
    for fname in sorted(os.listdir(DATA_DIR)):
        if not fname.endswith('.npy') or 'results' in fname:
            continue
        ...
        if tech is None:
            continue
        true_techs.append(tech)

    # labels_results is ordered by file listing order, match to true_techs
    # Both iterate os.listdir in the same order, so indices align
    # But labels_results only contains files that passed width gates
    # We need to rebuild from the CSV instead
    csv_path = os.path.join(DATA_DIR, 'df_result')
    if os.path.exists(csv_path):
        ...
        for _, row in df.iterrows():
            fname = row['fname']
            pred_label_str = row['label'] if isinstance(row['label'], str) else ''
            # Get predicted tech from the numeric label
            pred_num = float(row['label']) if not isinstance(row['label'], str) else -1
            # True tech from filename
            tech = None
            for t in LABELS:
                if t in fname.lower():
                    tech = t
            ...
            if tech is None:
                continue
            # Parse predicted tech from the dic lookup
            dic = {0: 'dab', 1: 'dvbt', 2: 'fm', 3: 'gsm', 4: 'lte', 5: 'tetra', 6: 'unkn'}
            pred_tech = dic.get(int(pred_num), 'unkn')
            true_techs.append(tech)
            pred_techs.append(pred_tech)

        # Filter known predictions (exclude unknown)
        known_mask = [p != 'unkn' for p in pred_techs]
        true_known = [t for t, m in zip(true_techs, known_mask) if m]
        pred_known = [p for p, m in zip(pred_techs, known_mask) if m]
        ...
        print(f"  Total classified: {len(pred_techs)}")
        print(f"  Unknown (filtered): {sum(1 for p in pred_techs if p == 'unkn')}")
        print(f"  Known predictions: {len(pred_known)}")
        ...
        # No-gate analysis: majority vote without entropy gate
        # (already stored in df — pred_techs without entropy filtering)
        all_cm = np.zeros((6, 6), dtype=int)
        for t, p in zip(true_techs, pred_techs):
            if t in LABEL_TO_IDX and p in LABEL_TO_IDX:
                all_cm[LABEL_TO_IDX[t], LABEL_TO_IDX[p]] += 1

        acc_all = np.trace(all_cm) / all_cm.sum() * 100 if all_cm.sum() > 0 else 0
        print(f"\n=== ALL PREDICTIONS (including unknown mapped to classes) ===")
        print(f"  Total: {len(pred_techs)}")
        print(f"  Overall Accuracy: {acc_all:.2f}% ({np.trace(all_cm)}/{all_cm.sum()})")
        ...
        ax.set_title(f'Authors Pipeline (TechClass.py)\nAccuracy: {acc:.1f}%', fontsize=12, fontweight='bold')
        plt.tight_layout()
        out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'figures', 'figure11_authors_pipeline.png')
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        plt.savefig(out_path, dpi=150, bbox_inches='tight')
        print(f"\nSaved: {out_path}")
    else:
        print(f"  WARNING: {csv_path} not found. inference_trx_labels() may have failed.")
